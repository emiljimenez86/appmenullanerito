(function () {
  ...
  const WHATSAPP = { penol: '573054705757', guatape: '573054705757' };
  const DIRECTIONS = {
    penol: {
  ...
  function renderPlatos(categoriaId) {
    ...
    cat.items.forEach(function (item) {
      ...
      html += '<button type="button" class="btn btn--add" data-nombre="' + escapeHtml(item.nombre) + '" data-precio="' + precio + '">Agregar a domicilio</button>';
      html += '</div></article>';
    });
    html += '</div>';
